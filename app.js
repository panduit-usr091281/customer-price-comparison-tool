...
function calculateClass2(powerW, distanceFt, labor, crewSize, installationType, outdoorType) {
  ...
  // Class 2: 60VDC system, 45V minimum at load → 15V max voltage drop
  // Current per pair at full load: 100W / 60V = 1.667A
  // Vdrop = I × 2 × R_per_ft × distance ≤ 15V
  // Copper resistance per foot (solid conductor):
  //   18AWG: 0.006385 Ω/ft
  //   16AWG: 0.004016 Ω/ft
  //   14AWG: 0.002525 Ω/ft
  //   12AWG: 0.001588 Ω/ft
  const maxVdrop = 15; // 60V - 45V minimum
  ...
  // Class 2 pairs: each pair delivers 100W (60V × 1.667A)
  const wattsPerPair = 100;
  const pairs = Math.max(1, Math.ceil(powerW / wattsPerPair));
  ...
  const lineItems = [
    ...
    createLineItem({
      phase: "2) Permitting and Preconstruction",
      activity: "Pre-design survey and coordination",
    ...
    createLineItem({
      ...
      laborRate: labor.lvTech,
      laborRole: "Low Voltage Technician",
      materials: [material("CL2 aggregators", injectorQty, "ea", 160)],
